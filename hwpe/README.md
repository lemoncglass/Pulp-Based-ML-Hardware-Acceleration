...
[`lemon_adder/`](lemon_adder/) — it adds two integers and is intentionally minimal
...
Copy `lemon_adder/` as a starting point:

```bash
cp -r hwpe/lemon_adder hwpe/my_accel
cd hwpe/my_accel
```
...
└── model/
    ├── lemon_adder.py        → rename to my_accel.py
    └── lemon_adder.cpp       → rename to my_accel.cpp
```
...
can usually be reused verbatim from the lemon_adder HAL — just rename the file
...
The lemon_adder model covers all of this in ~200 lines and is heavily commented.
...
Copy `lemon_adder.py` and change the `add_sources()` path.  The three port
