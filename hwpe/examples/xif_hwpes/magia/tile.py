"""
Magia Tile — System Wiring (Reference Copy)
=============================================

This is the Python "system builder" for one Magia tile.  GVSoC uses Python
scripts to describe the component hierarchy, port bindings, and memory maps.
At simulation start, GVSoC walks this tree and instantiates C++ components.

Each Magia tile contains:
  • CV32E40X core  — RISC-V core with eXtension Interface (XIF)
  • XifDecoder     — routes custom opcodes to the right coprocessor
  • LightRedmule   — matrix-multiply accelerator (this project!)
  • iDMA × 2       — DMA engines for data movement
  • TCDM            — tightly-coupled data memory (banked scratchpad)
  • I-cache         — instruction cache
  • Interconnects   — OBI and AXI crossbars for routing

Key wiring (XIF path):
  core.o_OFFLOAD  →  xifdec.i_OFFLOAD_M   (core sends unknown opcodes)
  xifdec.o_OFFLOAD_S2  →  redmule.i_OFFLOAD   (RedMulE-bound opcodes)
  redmule.o_OFFLOAD_GRANT  →  xifdec.i_OFFLOAD_GRANT_S2  →  core.i_OFFLOAD_GRANT

Original file: gvsoc/pulp/pulp/chips/magia/tile.py
Author: Lorenzo Zuolo (Chips-IT)
"""

# ---- Original source below (comments added for clarity) ----
...
# =========================================================================
#  MagiaTileTcdm — Banked scratchpad with 3 interleavers
# =========================================================================
#
#   The TCDM has three independent master interleavers:
#     1. L1_interleaver  (3 ports): OBI data, OBI debug, iDMA0/1
#     2. DmaInterleaver  (1 port) : iDMA direct access
#     3. HWPEInterleaver (1 port) : RedMulE direct access
#
#   All three connect to the same set of memory banks.
...
# =========================================================================
#  MagiaTile — One tile of the Magia chip
# =========================================================================
...
class MagiaTile(gvsoc.systree.Component):
    def __init__(self, parent, name, tree, parser, tid: int=0):
        ...
        # ---- RedMulE accelerator ----
        # Note the ce_height/ce_width parameters — these define the compute array size
        ...
        # =========================================================
        #  PORT BINDINGS — The key XIF wiring:
        # =========================================================

        # Core data ports → OBI crossbar
        ...
        # I-cache refill → AXI crossbar
        i_cache.o_REFILL(tile_xbar.i_INPUT())

        # ---- XIF decoder bindings (THE KEY PART) ----
        core_cv32.o_OFFLOAD(xifdec.i_OFFLOAD_M())             # Core → XifDecoder
        xifdec.o_OFFLOAD_GRANT_M(core_cv32.i_OFFLOAD_GRANT()) # XifDecoder → Core (grant)

        # XifDecoder → iDMA controller
        ...
        idma0.o_TCDM(l1_tcdm.i_INPUT(1))
        ...
        idma1.o_TCDM(l1_tcdm.i_INPUT(2))
        idma_ctrl.o_OFFLOAD_iDMA1_OBI2AXI(idma1.i_OFFLOAD())
        idma1.o_OFFLOAD_GRANT(idma_ctrl.i_OFFLOAD_GRANT_iDMA1_OBI2AXI())

        # ---- XifDecoder → RedMulE (THE KEY BINDING) ----
        redmule.o_TCDM(l1_tcdm.i_REDMULE_INPUT())            # RedMulE → TCDM
        xifdec.o_OFFLOAD_S2(redmule.i_OFFLOAD())              # XifDecoder → RedMulE
        redmule.o_OFFLOAD_GRANT(xifdec.i_OFFLOAD_GRANT_S2())  # RedMulE → XifDecoder
        redmule.o_IRQ(core_cv32.i_IRQ(31))                    # RedMulE → Core IRQ

        # ---- Memory map bindings ----
        # (OBI crossbar → TCDM, AXI crossbar, etc.)
        obi_xbar.o_MAP(l1_tcdm.i_INPUT(0), name="local-reserved",
                       base=MagiaArch.RESERVED_ADDR_START, size=MagiaArch.RESERVED_SIZE, rm_base=False)
        obi_xbar.o_MAP(l1_tcdm.i_INPUT(0), name="local-stack",
                       base=MagiaArch.STACK_ADDR_START, size=MagiaArch.STACK_SIZE, rm_base=False)
        obi_xbar.o_MAP(l1_tcdm.i_DMA_INPUT(), name="local-l1-mem",
        ...
        obi_xbar.o_MAP(tile_xbar.i_INPUT(), name="obi2axi-off-tile-l2-mem",
                       base=MagiaArch.L2_ADDR_START, size=MagiaArch.L2_SIZE, rm_base=False)
        ...
        obi_xbar.o_MAP(self.__i_KILLER_OUTPUT(), name="Kill-sim-mem",
                base=MagiaArch.TEST_END_ADDR_START, size=MagiaArch.TEST_END_SIZE, rm_base=False)
        obi_xbar.o_MAP(stdout.i_INPUT(), name="local-uart-mem",
                base=MagiaArch.STDOUT_ADDR_START, size=MagiaArch.STDOUT_SIZE, rm_base=False)
