...
def _load_hwpe_class(model_path):
    """Dynamically load a custom HWPE Component subclass from a Python file.

    Convention: the module must define exactly one ``gvsoc.systree.Component``
    subclass.  That class is returned so the caller can instantiate it.
    """
    ...
    for attr_name in dir(mod):
        ...
        if (isinstance(attr, type)
                and issubclass(attr, st.Component)
                and attr is not st.Component):
            return attr
    raise RuntimeError(f"No gvsoc.systree.Component subclass found in {model_path}")
...
class Cluster(st.Component):
    ...
    def __init__(self, parent, name, config_file, cid: int=0, pulpnn=False):
        ...
        custom_hwpe_model = self.declare_user_property(
            name='custom_hwpe', value='', cast=str,
            description='Path to custom HWPE Python model file (empty to disable)'
        )

        custom_hwpe_irq = cluster_conf.get_property('pe/irq').index('acc_0')  # shared HWPE IRQ slot
        ...
        if custom_hwpe_model:
            # CUSTOM HWPE (dynamically loaded from the path given via --target-property)
            _HwpeClass = _load_hwpe_class(custom_hwpe_model)
            custom_hwpe = _HwpeClass(self, 'custom_hwpe')
        ...
        if custom_hwpe_model:
            periph_ico.add_mapping('custom_hwpe', **self._reloc_mapping(cluster_conf.get_property('peripherals/custom_hwpe/mapping')))
            self.bind(periph_ico, 'custom_hwpe', custom_hwpe, 'input')
        ...
        if custom_hwpe_model:
            # CUSTOM HWPE
            for i in range(0, nb_pe):
                self.bind(custom_hwpe, 'irq', event_unit, 'in_event_%d_pe_%d' % (custom_hwpe_irq, i))

            self.bind(custom_hwpe, 'out', l1, 'custom_hwpe_in')
