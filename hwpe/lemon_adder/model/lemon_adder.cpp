 ...
 *
 * Lemon Adder HWPE — GVSoC simulation model (skeleton)
 *
 * This file implements the cycle-accurate C++ model that GVSoC loads
 * when the lemon_adder accelerator is enabled in the target config.
 *
 * === How this file works  ===
 *
 * SECTION 1 — Includes & constants
 *   Pull in the GVSoC component API and define the register offsets
 *   (must match archi_lemon_adder.h from the test side).
 *
...
/* ------------------------------------------------------------------ *
 * SECTION 1 — Register offsets (must match archi_lemon_adder.h)        *
 * ------------------------------------------------------------------ */

// Control registers
#define LEMON_ADDER_TRIGGER       0x00
#define LEMON_ADDER_ACQUIRE       0x04
#define LEMON_ADDER_FINISHED      0x08
#define LEMON_ADDER_STATUS        0x0C
#define LEMON_ADDER_RUNNING_JOB   0x10
#define LEMON_ADDER_SOFT_CLEAR    0x14

// Job registers (offsets relative to REG_OFFS)
#define LEMON_ADDER_REG_OFFS      0x40
#define LEMON_ADDER_REG_A_PTR     0x00 // will always be called as (REG_OFFS + <offset>)
#define LEMON_ADDER_REG_B_PTR     0x04 // ^
#define LEMON_ADDER_REG_RES_PTR   0x08 // ^

/* ------------------------------------------------------------------ *
 * SECTION 2 — Class definition                                       *
 * ------------------------------------------------------------------ */

class LemonAdder : public vp::Component
{
public:
    LemonAdder(vp::ComponentConf &config);
...
LemonAdder::LemonAdder(vp::ComponentConf &config) : vp::Component(config)
{
    // 1. Debug trace channel — lets us print messages with this->trace.msg(...)
    this->traces.new_trace("trace", &this->trace, vp::DEBUG);

    // 2. Master port to L1 memory (name "out" must match o_OUT in lemon_adder.py)
    ...
    //    (name "input" must match i_INPUT in lemon_adder.py)
    this->in.set_req_meth(&LemonAdder::hwpe_slave);
    this->new_slave_port("input", &this->in);

    // 5. Allocate the clock event for the computation FSM
    this->fsm_event = this->event_new(&LemonAdder::fsm_handler);

    this->trace.msg("LemonAdder build complete\n");
}
...
void LemonAdder::reset(bool active)
{
    if (active)
    {
        this->state = IDLE;
...
bool LemonAdder::access_mem(uint32_t addr, uint8_t *data, bool is_write)
{
    this->io_req.init();
...
vp::IoReqStatus LemonAdder::hwpe_slave(vp::Block *__this, vp::IoReq *req)
{
    ...
    LemonAdder *_this = (LemonAdder *)__this;
    ...
    if (is_write)
    {
        ...
        // WRITING DATA
        if (offset >= LEMON_ADDER_REG_OFFS)
        {
            // ---- Job registers (offset >= 0x40) ----
            uint32_t reg = offset - LEMON_ADDER_REG_OFFS;  // relative offset

            switch (reg)
            {
                case LEMON_ADDER_REG_A_PTR:
                    _this->trace.msg("Write REG_A_PTR = 0x%x\n", value);
                    _this->reg_a_ptr = value;
                    break;

                case LEMON_ADDER_REG_B_PTR:
                    _this->trace.msg("Write REG_B_PTR = 0x%x\n", value);
                    _this->reg_b_ptr = value;
                    break;

                case LEMON_ADDER_REG_RES_PTR:
        ...
        else
        {
            ...
            switch (offset)
            {
                case LEMON_ADDER_TRIGGER: // Set state to running
                ...
                case LEMON_ADDER_SOFT_CLEAR: // Reset everything
                ...
                case LEMON_ADDER_ACQUIRE:     break;
                case LEMON_ADDER_FINISHED:    break;
                case LEMON_ADDER_STATUS:      break;
                case LEMON_ADDER_RUNNING_JOB: break;
    ...
    else
    {
        ...
        switch (offset)
        {
            case LEMON_ADDER_ACQUIRE: // can I accept a job request?
                // 0 = free (job slot available), nonzero = busy
                *data = (_this->state == IDLE) ? 0 : 1;
                _this->trace.msg("Read ACQUIRE → %d\n", *data);
                break;

            case LEMON_ADDER_STATUS: // what state am I in?
                // 0 = idle, 1 = running
                *data = (_this->state == RUNNING) ? 1 : 0;
                _this->trace.msg("Read STATUS → %d\n", *data);
                break;

            case LEMON_ADDER_FINISHED: // tell core I'm done with all jobs
...
void LemonAdder::fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    LemonAdder *_this = (LemonAdder *)__this;
