...
/* HAL & register map (includes archi_toy_adder.h automatically) */
#include "hal_toy_adder.h"
...
{
    printf("\n--------------------------------------\n");
    printf("[toy_adder] Operand A (L1) = %d\n", operand_a);
    printf("[toy_adder] Operand B (L1) = %d\n", operand_b);

    /* ---------- Program the HWPE registers via HAL -------------- */
    // Pass the L1 addresses of our operands and result buffer
    toy_adder_set_a((unsigned int)(intptr_t)&operand_a);
    toy_adder_set_b((unsigned int)(intptr_t)&operand_b);
    toy_adder_set_res_ptr((unsigned int)(intptr_t)&result);

    /* ---------- Launch the accelerator and wait for it ---------- */
    // TRIGGER tells the HWPE to start; the core then sleeps until
    // the HWPE fires its IRQ on event line TOY_ADDER_EVT0 (acc_0).
    hwpe_trigger_job();
    eu_evt_maskWaitAndClr(1 << TOY_ADDER_EVT0);

    /* ---------- Verify against golden output --------------------- */
    printf("[toy_adder] Result         = %d\n", result);
    printf("[toy_adder] Golden         = %d\n", GOLDEN_SUM);
...
static int test_entry(void)
{
    ...
    {
        printf("[toy_adder] ERROR: could not open cluster\n");
        return -1;
    }
